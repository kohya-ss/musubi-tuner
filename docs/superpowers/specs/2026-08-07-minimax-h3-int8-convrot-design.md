...
The implementation keeps Krea2's working on-load quantization contract intact. Shared code is added only for Comfy-style prequantized artifact inspection and per-layer runtime patching. A Krea2 checkpoint quantized at load time may continue to use `scale_weight`; a Comfy artifact keeps `weight_scale`.
...
- `convrot is True`
- A positive power-of-two `convrot_groupsize`
- `in_features % convrot_groupsize == 0`
- A Hadamard group size supported by the shared ConvRot kernel

Floating-point layers without `.comfy_quant` are valid mixed-precision islands. An INT8 weight without a complete and valid sibling triple is invalid.
...
Inspection happens before the meta model is populated. It reads normal tensor headers and only materializes the small U8 control tensors. Key prefixes and architecture key transforms are applied consistently to all three sibling keys.

The adapter prepares a model by locating every declared module, requiring an exact `nn.Linear`, replacing its meta weight contract with frozen INT8 storage, registering the declared scale buffer, storing the per-module group size, and binding the shared ConvRot forward. The existing Krea2 helper remains callable with its state-dict-driven API and creates equivalent layer specs internally.

The shared forward resolves the scale buffer selected during preparation. Krea2 can retain `scale_weight`; Comfy artifacts retain `weight_scale`. `ConvRotInt8LinearFn` already accepts group size as an argument, so the numerical kernel does not need an architecture branch.
...
2. Normalize raw keys and inspect the ConvRot sibling triples.
...
6. Stream weight and scale tensors shard by shard with `assign=True`.
...
Strict dtype checking understands the inspected INT8 layers. It still checks source dtype for all fixed precision islands. Duplicate normalized keys across shards remain errors.
...
```python
position = t.clamp(0.0, 1.0) * (table.shape[0] - 1)
...
Standard AdaLN continues to apply SiLU inside `AdalnProj`. Pruned AdaLN consumes the interpolated curve coordinate directly and skips that SiLU. Endpoint behavior at `t=0` and `t=1` is part of the numerical contract.
...
Quantized Linears are excluded from `torch.compile`, matching Krea2. Block swap moves both frozen INT8 weights and FP32 scale buffers with their owning block. Per-layer group sizes are plain immutable attributes and require no device transfer.

### 9.3 Standalone and scheduled generation

Generation uses the same automatic transformer detection. BF16 transformers retain the existing destructive LoRA merge. INT8 transformers create inference LoRA modules, apply them as additive branches, load their state dicts, and keep the modules alive for the sampling lifetime. Multiple requested LoRAs remain independent additive branches with their configured multipliers.

Training-time scheduled samples already execute through the attached training network and need no separate merge path.

### 9.4 Text encoder

`load_h3_text_encoder` applies the artifact adapter after the existing Qwen config truncation and layer-50 final-norm removal. INT8 is accepted only for valid ConvRot triples. The encoder remains frozen and evaluation-only, so no text-encoder backward mode is exposed.
...
- Unsupported group size or indivisible input width.
...
- Krea2 `scale_weight` compatibility.
...
- Block swap moves INT8 weights and scales together.
