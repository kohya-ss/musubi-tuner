...
- **Audio education is only as good as the dataset's audio.** The teacher copies each clip's actual audio track, and with the default `--audio_loss_weight 1.0` the audio term carries a large share of the education loss (measured: over half). A consistent voice across clips is learnable signal; but when the training audio is not consistent (e.g. synthesized clips generated without an audio reference), the per-clip audio content is unpredictable from text, the audio residual stops shrinking after the easy energy/DC components, and the remainder acts as sustained gradient noise on the shared single-stream weights — measured to slowly degrade video quality once the video-side residual has plateaued. Lower `--audio_loss_weight` (e.g. 0.1-0.25; it also scales the audio preservation anchor, so keeping it nonzero retains some base-audio protection) or pass `--video_only` for such data.
- The teaching-band residual can plateau well before a long run ends (measured around 300 steps on a small character set); training past the plateau mostly optimizes irreducible floors and can degrade samples. Save and validate intermediate checkpoints rather than judging only the final one.
