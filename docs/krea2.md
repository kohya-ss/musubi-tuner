...
- **DiT (RAW)**: The MMDiT transformer model, full-step version. This is the recommended LoRA training base and can also be full-finetuned.
- **DiT (Turbo)** *(optional)*: The distilled few-step version. Used for inference and optional LoRA sample generation; it can also be selected as the primary full-finetuning checkpoint.
...
- **DiT (RAW)**: MMDiT transformerモデルのフルステップ版。LoRA学習の推奨ベースであり、DiT全体のファインチューニングにも使用できます。
- **DiT (Turbo)** *（オプション）*: 蒸留された少ステップ版。推論と任意のLoRAサンプル生成に使用し、DiT全体のファインチューニングではprimary checkpointとして選択することもできます。
...
LoRA training uses the dedicated script `krea2_train_network.py`. Training on the **RAW** DiT remains the recommended workflow.
...
Full finetuning uses `krea2_train.py` and trains every parameter of the primary Krea 2 DiT. Run latent and text-encoder-output pre-caching before training.

```bash
accelerate launch --num_cpu_threads_per_process 1 --mixed_precision no krea2_train.py \
    --dit path/to/raw_dit_model \
    --dit_variant raw \
    --dataset_config path/to/toml \
...
- `--dit_variant raw` (the default) selects the resolution-aware RAW sampling schedule. To finetune a Turbo checkpoint as the primary model, pass that checkpoint to `--dit` and set `--dit_variant turbo`; training-time samples then use fixed `mu=1.15` and should use CFG off with a low step count.
- Full finetuning always samples from the current trainable primary checkpoint. It rejects the LoRA-only `--turbo_dit` and `--turbo_dit_cache` weight-swap options because restoring a frozen RAW snapshot would overwrite learned parameters.
...
DiT全体のファインチューニングには`krea2_train.py`を使用し、primary Krea 2 DiTの全パラメータを学習します。学習前にlatentとテキストエンコーダー出力を事前キャッシュしてください。コマンド例は英語版を参照してください。

- `--dit_variant raw`（デフォルト）は、解像度依存のRAWサンプリングスケジュールを選択します。Turboチェックポイントをprimary modelとしてファインチューニングする場合は、そのチェックポイントを`--dit`に渡し、`--dit_variant turbo`を指定してください。学習中のサンプルでは固定`mu=1.15`が使用されるため、CFGをオフにして少ないステップ数を指定します。
- DiT全体のファインチューニングでは、常に現在学習中のprimary checkpointからサンプルを生成します。LoRA専用の`--turbo_dit`と`--turbo_dit_cache`による重みスワップは拒否されます。凍結RAWのスナップショットを復元すると、学習済みパラメータが上書きされるためです。
...
- For LoRA training, `--fp8_base` and `--fp8_scaled` reduce DiT memory usage. **Both must be specified together** (plain fp8 without scaled is rejected, because it would cast the norms to fp8 and break the model). fp8 is applied to the 28 main blocks only; the text-fusion transformer stays bf16. Full finetuning does not support DiT FP8.
...
- LoRA学習では、`--fp8_base`と`--fp8_scaled`でDiTのメモリ使用量を削減します。**両方を同時に指定する必要があります**（scaledなしのplain fp8は、normをfp8にキャストしてモデルを壊すため拒否されます）。fp8は28個のメインブロックのみに適用され、text-fusion transformerはbf16のまま保持されます。DiT全体のファインチューニングではDiT FP8は使用できません。
...
For the recommended RAW LoRA workflow, samples are generated on the RAW model being trained by default, using CFG (specify a negative prompt and a CFG scale via `--l` in the sample prompt; CFG-off output is blurry, which is expected for the K2 RAW model).
...
The `--turbo_dit` options above are LoRA-only. Full finetuning samples the current `--dit` without swapping weights; `--dit_variant raw|turbo` selects the matching schedule.
...
推奨されるRAW LoRAワークフローでは、デフォルトで学習中のRAWモデルからCFGを使用してサンプルが生成されます（サンプルプロンプトでネガティブプロンプトと`--l`によるCFGスケールを指定してください。CFGなしの出力はぼやけますが、これはK2 RAWモデルでは想定通りです）。
...
上記の`--turbo_dit`オプションはLoRA専用です。DiT全体のファインチューニングでは重みを入れ替えず、現在の`--dit`からサンプルを生成し、`--dit_variant raw|turbo`で対応するスケジュールを選択します。
