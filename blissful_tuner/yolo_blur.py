...
parser = setup_parser_video_common(description="Blur faces in images/videos with LoRA for training face agnostic LoRA", model_help="Path to yolo face model e.g. yolov8x-face-lindevs.pt")
parser.add_argument("--strength", required=False, type=int, default=51, help="Strength of blur. Higher values make the blur more intense")
parser.add_argument("--sigmax", required=False, type=int, default=40, help="SigmaX of blur. The standard deviation in the X direction")
parser.add_argument("--conf", type=float, default=0.5, help="Confidence threshold for face detection. Higher means less false positives, but might miss hard to detect faces.")
...
vp.prepare_files_and_path(args.input, args.output, "blur", args.codec, args.container)
