...
class FlowMatchLCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        x0_pred = sample.to(self.sigmas.device) - sigma * model_output.to(self.sigmas.device)
...
class FlowMatchEulerAncestralDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def scale_model_input(self, sample: torch.Tensor, timestep: Union[float, torch.Tensor]) -> torch.Tensor:
        ...
        sigma = self.sigmas[self.step_index]
        cur_device = sample.device  # Move to device and back because sample may be on cpu
        calc_device = self.sigmas.device
        sample = (sample.to(calc_device) / ((sigma**2 + 1) ** 0.5)).to(cur_device)
    ...
    def step(
        ...
        pred_original_sample = sample.to(device) - sigma * model_output.to(device)
        ...
        derivative = (sample.to(device) - pred_original_sample) / sigma

        dt = sigma_down - sigma

        prev_sample = sample.to(device) + derivative * dt

        noise = randn_tensor(model_output.shape, dtype=model_output.dtype, device=generator.device, generator=generator).to(device)
        self.last_noise = noise
