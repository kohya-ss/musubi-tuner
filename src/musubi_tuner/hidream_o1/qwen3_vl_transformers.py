...
def auto_docstring(*args, **kwargs):
    """No-op replacement for Transformers' docstring decorator.

    The upstream decorator may print generated warning text during import. On
    non-UTF-8 Windows consoles that can raise UnicodeEncodeError before the
    model class is usable. Runtime behavior does not depend on generated docs.
    """
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def switch_block_swap_for_inference(self):
        if self.blocks_to_swap:
            ...
            print("HiDream-O1: Block swap set to forward only.")

    def switch_block_swap_for_training(self):
        if self.blocks_to_swap:
            self.offloader.set_forward_only(False)
            self.prepare_block_swap_before_forward()
            print("HiDream-O1: Block swap set to forward and backward.")
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.blocks_to_swap:
                self.offloader.submit_move_blocks_forward(self.layers, layer_idx)
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def _run_decoder_flash(
        ...
        Args:
            ...
            visual_pos_masks: [batch, total_seq_len] visual placeholder positions for DeepStack
        ...
        for layer_idx, decoder_layer in enumerate(text_model.layers):
            if text_model.blocks_to_swap:
                text_model.offloader.wait_for_block(layer_idx)
            ...
            if text_model.blocks_to_swap:
                text_model.offloader.submit_move_blocks_forward(text_model.layers, layer_idx)
    ...
    def _forward_generation(self, input_ids, position_ids, vinputs, timestep, token_types,
        ...
        cond_deepstack_image_embeds_out: Optional[list[torch.Tensor]] = None

        # 1. Get text token embeddings
        if inputs_embeds is None:
            inputs_embeds = self.get_input_embeddings()(input_ids)  # [batch, txt_seq_len, hidden]
        else:
            inputs_embeds = inputs_embeds.to(self.get_input_embeddings().weight.device)
        ...
        if pixel_values is not None:
            ...
            for fake_deepstack_embed in fake_deepstack:
                fake_total = fake_total + fake_deepstack_embed.to(inputs_embeds.dtype).sum()
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def switch_block_swap_for_training(self):
        self.language_model.switch_block_swap_for_training()

    def move_to_device_except_swap_blocks(self, device: torch.device):
        if self.blocks_to_swap:
            language_model = self.model.language_model
            save_layers = language_model.layers
            language_model.layers = nn.ModuleList()

        self.to(device)

        if self.blocks_to_swap:
            language_model.layers = save_layers

    def prepare_block_swap_before_forward(self):
        self.language_model.prepare_block_swap_before_forward()
