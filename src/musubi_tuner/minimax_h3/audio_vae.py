...
# module uses ordinary torch Conv1d / ConvTranspose1d layers and loads the
# converted checkpoint's plain "*.weight" tensors through the shared loader.
#
...
def encode_audio_mode(vae, waveform: torch.Tensor) -> torch.Tensor:
    ...
    if not isinstance(latents, torch.Tensor) or latents.ndim != 4 or latents.shape[1:3] != (32, 2):
        shape = tuple(latents.shape) if isinstance(latents, torch.Tensor) else type(latents).__name__
        raise ValueError(f"Expected MiniMax-H3 audio latent [B,32,2,A], got {shape}")
    return latents
