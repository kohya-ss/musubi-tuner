...
2. If it is not installed, a lightweight stub implemented in
   :mod:`musubi_tuner.utils._cv2_stub` is registered as ``cv2`` in
...
a missing OpenCV installation::
    ...
    from musubi_tuner.utils import cv2_compat  # noqa: F401 - must come before `import cv2`
    import cv2
...
try:
    ...
    if "cv2" not in sys.modules:
        from musubi_tuner.utils import _cv2_stub
