...
by :mod:`musubi_tuner.utils.cv2_compat` when the real ``opencv-python`` package is not
...
detection, imdecode/imencode, etc.) check :data:`musubi_tuner.utils.cv2_compat.HAS_OPENCV`
...
def cvtColor(src: np.ndarray, code: int) -> np.ndarray:
    ...
    raise NotImplementedError(f"cv2 stub: cvtColor code {code} is not implemented. Install opencv-python to use this conversion.")
...
# Sentinel that :mod:`musubi_tuner.utils.cv2_compat` reads to decide whether the caller
