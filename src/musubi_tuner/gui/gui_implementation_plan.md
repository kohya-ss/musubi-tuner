※このファイルはGUIの実装方針を記載するためのものです。実装が終わったら削除します。
...
Qwen-Image、Z-Imageなどのモデルアーキテクチャを選択します。最初は画像生成モデルのみ対応を予定し、まずはZ-Imageを対象に実装します（選択リストに `Z-Image` だけを配置します）。

モデルディレクトリは、ComfyUI の `models` フォルダをユーザーに指定してもらいます。サブフォルダ（`diffusion_models`や`vae`、`text_encoders`）とモデルファイル名は自動で決定します。（見つからなかったらエラー表示。）

3. データセット設定

学習解像度等を指定しデータセット設定ファイルを作成します。

学習解像度はモデル選択時に自動決定し（たとえばZ-Imageなら1024x1024、Qwen-Imaageなら1328x1328）、ユーザーが変えられるようにします。

解像度以外は、バッチサイズのみ指定可能とします。

4. 前処理

学習データの前処理、具体的にはlatentの事前キャッシュと、text encoderの事前キャッシュを行います。
...
`musubi_project.toml` に以下の項目を保存し、次回読み込み時に復元します。
