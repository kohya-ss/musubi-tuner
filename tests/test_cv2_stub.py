...
def test_stub_is_registered_when_opencv_is_missing():
    code = textwrap.dedent(
        ...
        import musubi_tuner.dataset.media_utils  # imports cv2_compat before cv2
        import cv2
        from musubi_tuner.utils import cv2_compat
...
def test_require_opencv_raises_without_opencv(monkeypatch):
    ...
    with pytest.raises(SystemExit):
        cv2_compat.require_opencv("some tool")
    monkeypatch.setattr(cv2_compat, "HAS_OPENCV", True)
    cv2_compat.require_opencv("some tool")  # no-op
