...
DIT_LOW_FILE="${MODEL_DIR}/dit/wan2.2_t2v_low_noise_14B_fp16.safetensors"
if [ -f "${DIT_LOW_FILE}" ]; then
    ...
    if [ -f "${MODEL_DIR}/dit/split_files/diffusion_models/wan2.2_t2v_low_noise_14B_fp16.safetensors" ]; then
        mv "${MODEL_DIR}/dit/split_files/diffusion_models/wan2.2_t2v_low_noise_14B_fp16.safetensors" "${DIT_LOW_FILE}"
...
DIT_HIGH_FILE="${MODEL_DIR}/dit/wan2.2_t2v_high_noise_14B_fp16.safetensors"
if [ -f "${DIT_HIGH_FILE}" ]; then
    ...
    huggingface-cli download \
        ...
        --local-dir-use-symlinks False
    # Move from nested directory
    if [ -f "${MODEL_DIR}/dit/split_files/diffusion_models/wan2.2_t2v_high_noise_14B_fp16.safetensors" ]; then
        mv "${MODEL_DIR}/dit/split_files/diffusion_models/wan2.2_t2v_high_noise_14B_fp16.safetensors" "${DIT_HIGH_FILE}"
        rm -rf "${MODEL_DIR}/dit/split_files"
    fi
