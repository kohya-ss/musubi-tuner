...
- Save generation metadata in videos/images (automatic with `--container mkv`, disable with `--no-metadata`, not available with `--container mp4` You can view/copy such metadata with `blissful_tuner/metaview.py some_video.mkv`)
