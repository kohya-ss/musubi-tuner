...
- August 3, 2026
    - Added experimental MiniMax-H3 support for T2VA, FL2VA, and Ref2VA LoRA training plus standalone and scheduled training-time joint video/audio generation. H3 supports the published BF16 transformers, dual VAEs, Qwen3-VL-32B conditioning, block swap, FL2VA image-style LoRA modes (`first`, `first_last`, and `multiple_target`), and experimental Comfy-Org quantized artifacts for local cache/training workflows. See the [MiniMax-H3 documentation](./docs/minimax_h3.md) for dataset, cache, training, and generation details.
